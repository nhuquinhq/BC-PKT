...
/* url có thể là một chuỗi hoặc MẢNG đường đọc — /api/sheet thử lần lượt tới
   khi có đường ra. Cùng một tab mà Google lúc xuất được đường này lúc đường
   kia, nên khai hai đường thì trang đỡ trắng. */
export async function fetchSheetGrid(url, gid) {
  const qs = new URLSearchParams({ raw: '1' });
  for (const u of (Array.isArray(url) ? url : [url]).filter(Boolean)) qs.append('url', u);
  if (gid) qs.set('gid', gid);
  const res = await fetch(`/api/sheet?${qs.toString()}`, { cache: 'no-store' });
  const json = await res.json();
  if (!res.ok) throw new Error(json.error || 'Không đọc được Google Sheet');
