...
export async function nhoDoc(khoa, tao, { song = SONG, hanCu = HAN_CU, kv: dungKV = true, moi = false, songLoi = SONG_LOI } = {}) {
  ...
  /* Vừa hỏng xong mà cũng chẳng có bản cũ nào để hiện: báo lỗi luôn. Ngồi đợi
     ...
     TRỪ khi bên gọi xin đọc lại cho bằng được (moi=1) — đó là người dùng bấm
     thử lại, hoặc bot xin số mới. Trước đây moi chỉ bỏ qua bản nhớ GIÁ TRỊ chứ
     không bỏ qua bản nhớ LỖI, nên một lượt trượt là trang kẹt lỗi nguyên 5
     phút và bấm thử lại bao nhiêu lần cũng vẫn ra đúng lỗi cũ. */
  const loiCu = loiGanDay.get(khoa);
  if (!moi && !luu && loiCu && Date.now() - loiCu.luc < songLoi) throw new Error(loiCu.msg);
