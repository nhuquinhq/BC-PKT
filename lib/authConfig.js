...
import { TEAM_SAN_MAP } from '@/lib/cpvDims';
...
/* ------------------------------------------------------------------
   Phân quyền 2 cấp: admin cấp cho LEADER xem theo BU — chỉ cần tick
   trang team (vd HQS200) là TỰ KÉO THEO các trang SÀN của BU đó
   (map TEAM_SAN_MAP); leader cấp lại đúng các trang SÀN mình được
   xem cho NHÂN VIÊN. Trang sàn nhận diện qua tiền tố mã. */
export const SAN_PAGE_PREFIX = 'SÀN ';

/* Trang team kéo theo các trang SÀN thuộc BU đó */
export const expandTeamSan = (pages) => {
  if (!Array.isArray(pages)) return pages; /* 'all' | null giữ nguyên */
  const out = new Set(pages);
  for (const p of pages) for (const san of TEAM_SAN_MAP[p] || []) out.add(SAN_PAGE_PREFIX + san);
  return [...out];
};
...
export const canViewPage = (me, code) => {
  ...
  const pages = expandTeamSan(me.pages);
  return Array.isArray(pages) && pages.includes(code);
};

/* Danh sách mã trang mà một tài khoản được phép cấp cho người khác
   (pages đã resolve qua resolvePages). Admin: mọi trang · Leader:
   chỉ các trang SÀN trong quyền xem của mình (gồm sàn kéo theo từ
   trang team) · còn lại: không. */
export const grantablePages = (role, pages) => {
  if (role === 'admin') return PAGES.map((p) => p.code);
  if (role !== 'leader') return [];
  const own = pages === 'all' ? PAGES.map((p) => p.code) : expandTeamSan(Array.isArray(pages) ? pages : []);
