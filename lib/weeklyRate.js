...
export function parseWeeklyRate(grid) {
  ...
  /* ---------- Bảng tidy cho weekly_rate + KPI ----------
     Tìm trên TOÀN BỘ dòng, không bó trong khu "RATE THIS WEEK" nữa: tab
     WEEKLY RATE Ver2 tách thành ba khu — RATE THIS WEEK, RATE REVENUE (chứa
     REV Rate) và RATE CO (chứa CO Rate). Lọc theo khu cũ thì rowRev/rowCo
     đều rỗng, kéo theo mọi thẻ KPI của PKT1 trắng dù bảng bên dưới đủ số.
     Lấy dòng KHỚP ĐẦU TIÊN, tức khối tỉ giá áp dụng ở đầu bảng, chứ không
     phải mấy khối theo dõi chi tiết ER1 lặp lại ở cuối. */
  const find = (fn) => outRows.find(fn) || null;
  /* Ver1 ghi chiều RUB/USDT, Ver2 ghi ngược thành USDT/RUB — nhận cả hai để
     đổi tab không phải sửa lại chỗ này. */
  const rowRub = find((r) => r.main && (r.currency === 'RUB/USDT' || r.currency === 'USDT/RUB'));
  const rowRubVnd = find((r) => !r.main && packed(r.kind).startsWith('RUB/VND'));
  const rowIdr = find((r) => r.main && (r.currency === 'IDR/USDT' || r.currency === 'USDT/IDR'));
  ...
  /* Tuần dùng cho KPI là tuần cuối CÓ TỶ GIÁ, không phải cột cuối cùng.
     Sheet kẻ sẵn khối cột cho tuần chưa tới: đo ngày 07/09 thì cột cuối là
     tuần 38 (14/9–20/9) còn trống, lấy thẳng cột cuối là mọi thẻ KPI trên
     PKT1 trắng trơn dù bảng bên dưới vẫn đủ số. */
  const coSo = (r) => r && (r.usdt_vnd_rev != null || r.usdt_vnd_co != null);
  const last = [...tidy].reverse().find(coSo) || tidy[tidy.length - 1] || {};
  const kpis = {};
  ...
  return { weeks, rows: outRows, tidy, kpis, tuanKpi: last.tuan || '' };
}

/* ---------- Nhận xét tuần: so sánh tuần mới nhất với tuần liền trước ---------- */
const COMPARE_SPECS = [
  { key: 'rub_usdt', label: 'RUB/USDT' },
  { key: 'idr_usdt', label: 'IDR/USDT' },
  { key: 'usdt_vnd_co', label: 'USDT/VND · CO Rate' },
  { key: 'usdt_vnd_rev', label: 'USDT/VND · REV Rate' },
  { key: 'usd_vnd', label: 'USD/VND' },
  { key: 'rub_vnd', label: 'RUB/VND' },
  { key: 'idr_vnd', label: 'IDR/VND' },
  { key: 'eur_usdt', label: 'USDT/EUR' },
];
...
export function buildWeeklyCompare(tidy) {
  /* Chỉ so các tuần ĐÃ CÓ tỷ giá — sheet kẻ sẵn cột tuần chưa tới, so với cột
     rỗng thì mọi dòng ra "—" trong khi thực tế có thay đổi. */
  const co = (tidy || []).filter((r) => r && (r.usdt_vnd_rev != null || r.usdt_vnd_co != null));
  if (co.length < 2) return null;
  const cur = co[co.length - 1];
  const prev = co[co.length - 2];

  const items = [];
  let up = 0;
  let down = 0;
  let flat = 0;
  for (const s of COMPARE_SPECS) {
    const a = prev[s.key];
    const b = cur[s.key];
    ...
    items.push({
      ...
      curTxt: fmtN(b),
      prevTxt: fmtN(a),
      deltaTxt: `${delta > 0 ? '+' : ''}${fmtN(delta)}`,
      pctTxt: `${pct > 0 ? '+' : ''}${pct.toLocaleString('vi-VN', { maximumFractionDigits: 2 })}%`,
      dir,
    });
  }
  if (!items.length) return null;

  const gap = cur.gap_2_tuan;
  return {
    cur: { week: cur.tuan, from: cur.tu_ngay, to: cur.den_ngay },
    prev: { week: prev.tuan, from: prev.tu_ngay, to: prev.den_ngay },
    items,
    counts: { up, down, flat },
    gap:
      gap == null
        ? null
        : {
            pctTxt: `${gap.toLocaleString('vi-VN', { maximumFractionDigits: 2 })}%`,
            breach: Math.abs(gap) > 2,
          },
  };
}
