/* ============================================================
   ...
   Quan hệ giữa hai cái KHÔNG phải xấp xỉ mà là công thức cứng trên file:
     REV = CO × 98%
   (ô REV tuần 37 là =EH31*98%). Đúng nguyên bảng: 26.070×98%=25.549 ·
   25.814×98%=25.297 · 25.803×98%=25.286 · 25.907×98%=25.389. Dùng được làm
   phép thử nhanh: bóc ra mà REV ≠ CO×98% thì một trong hai cột đã lấy nhầm.
...
/* HAI đường đọc cùng một tab "WEEKLY RATE Ver2" (gid 1739295342), thử lần lượt.
   Đo ngày 07/09:
   - Bản công bố: ra, 76 dòng × 361 cột, bóc được 19 tuần.
   - gviz trên file gốc: cũng ra, 49 dòng × 361 cột, ra ĐÚNG 19 tuần đó với số
     y hệt. Ít dòng hơn vì gviz lược bớt dòng rỗng — nên bocBang() bám NHÃN
     dòng chứ không bám số thứ tự dòng, cùng bảng mà "CO Rate" nằm dòng 30 ở
     đường công bố và dòng 19 ở gviz.
   - /export trên file gốc thì KHÔNG: trả trang đăng nhập của Google vì file
     chưa mở quyền "ai có link". Nên đường dự phòng là gviz, không phải export.
   Một đường hụt còn đường kia; hụt cả hai mới lùi về bảng tĩnh. */
const DUONG_TY_GIA = [
  ...
  'https://docs.google.com/spreadsheets/d/1tMLW_mB8U08CQvE-lPBoLYAbjjs2Cc78t5bLHWTsK48/gviz/tq?tqx=out:csv&gid=1739295342&headers=0',
];
...
async function docBang() {
  try {
    const { val } = await nhoDoc(
      ...
      async () => {
        const truot = [];
        for (const url of DUONG_TY_GIA) {
          try {
            const res = await fetch(url, { redirect: 'follow', cache: 'no-store', signal: AbortSignal.timeout(20000) });
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            const text = await res.text();
            /* Trang đăng nhập của Google cũng trả HTTP 200 — không bắt ở đây
               thì bocBang() ném lỗi khó hiểu thay vì nhảy sang đường sau. */
            if (text.trim().startsWith('<')) throw new Error('nhận về HTML thay vì CSV');
            return bocBang(text);
          } catch (e) {
            truot.push(`${url.includes('/gviz/') ? 'gviz' : 'công bố'}: ${e.message}`);
          }
        }
        throw new Error(`Cả ${DUONG_TY_GIA.length} đường đều hụt (${truot.join(' · ')})`);
      },
      { song: 6 * 3600 * 1000 }
    );
