...
export function timTieuDe(grid) {
  for (let i = 0; i < Math.min(grid.length, 20); i++) {
    ...
    const coSan = timCot(h, 'ten san') >= 0 || timCot(h, 'san') >= 0;
    if (coSan && timCot(h, 'so tien') >= 0 && timCot(h, 'tim') >= 0) return { headIdx: i, headers: h };
  }
...
export function docCot(headers) {
  return {
    san: timCot(headers, 'ten san') >= 0 ? timCot(headers, 'ten san') : timCot(headers, 'san'),
...
export function parseWallet(grid, { month, year }) {
  ...
  return { detail, ok, usdTong, vndTong, headIdx, col };
}
