...
export function sheetQuery(cfg) {
  ...
  const themFile = (f) => {
    ...
    /* urlb: đường đọc DỰ PHÒNG của CÙNG tab đó (không phải file khác). Cùng
       một tab mà Google xuất được đường này lại hụt đường kia, nên khai hai
       đường thì trang đỡ hỏng theo xác suất. Append theo từng file để server
       ghép đúng cặp với url. */
    qs.append('urlb', f.urlb || '');
