...
const VI_SHEET_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQumf3iUT9n-lh-sNiJ1TeV24RjqRm1Eo6RTMafFsN9g_Hr6qtKZfyjFzb4gBpo5QhTieMSqU4sJORx/pubhtml?gid=1106881524&single=true';
...
const cpvTeamReport = (code, bu) => ({
  ...
  sheetVi: { url: VI_SHEET_URL, gid: '1106881524', mode: 'order_cpv', endpoint: '/api/vi', qs: { month: '7', year: '2026' }, hist: true, teamFilter: code, label: `Lịch sử ví · Team ${code}` },
  purpose: [
...
const cpvSanReport = (san) => ({
  ...
  sheetVi: { url: VI_SHEET_URL, gid: '1106881524', mode: 'order_cpv', endpoint: '/api/vi', qs: { month: '7', year: '2026' }, hist: true, sanFilter: san, label: `Lịch sử ví · Sàn ${san}` },
  purpose: [
...
export const REPORTS = [
  ...
  /* ---------------------------------------------------------- PKT6 */
  {
    ...
    sheet: {
      ...
      mode: 'order_cpv',
      endpoint: '/api/vi',
      qs: { month: '7', year: '2026' },
      hist: true,
      label: 'Lịch sử ví HQS10000 · tab THVí Tiền',
    },
    purpose: [
    ...
    tables: [
      {
        ...
        totals: true,
        hint: 'T1–T7 lấy bản chốt tháng của file (sheet Monthly.Report). Tháng đang chạy đọc trực tiếp từ tab giao dịch — cột Nguồn ghi rõ từng dòng.',
        columns: [
          { key: 'thang', label: 'Tháng', type: 'text' },
          { key: 'gmv', label: 'GMV', type: 'money' },
      ...
      {
        id: 'cpv_nhom',
        title: 'GMV theo NHÓM HÀNG (Giftcard · Tiện ích · Steam)',
        totals: true,
        hint: 'Bóc từ bản chốt tháng — Ritokey từ T7 gần như chỉ còn Giftcard.',
        columns: [
          { key: 'thang', label: 'Tháng', type: 'text' },
          { key: 'gmv_giftcard', label: 'Giftcard', type: 'money' },
          { key: 'gmv_tien_ich', label: 'Tiện ích', type: 'money' },
          { key: 'gmv_steam', label: 'Steam', type: 'money' },
      ...
      {
        id: 'cpv_ngay',
        title: 'Chi tiết theo NGÀY — tháng đang chạy',
        totals: true,
        hint: 'Đọc trực tiếp tab giao dịch đã công bố, chỉ tính đơn Thành công / Hoàn tất theo ngày hoàn tất.',
        columns: [
          { key: 'ngay', label: 'Ngày', type: 'text' },
          { key: 'gmv', label: 'GMV', type: 'money' },
          { key: 'co', label: 'Giá vốn', type: 'money' },
          { key: 'pl1', label: 'PL1', type: 'money' },
          { key: 'ty_le_co', label: '% CO/GMV', type: 'pct', totalOf: ['co', 'gmv'] },
          { key: 'so_don', label: 'Số đơn', type: 'num' },
        ],
      },
    ],
    charts: [
      { id: 'c_thang', title: 'GMV – Giá vốn – PL1 theo THÁNG', type: 'bar', table: 'cpv_thang', x: 'thang', series: [{ key: 'gmv', label: 'GMV' }, { key: 'co', label: 'Giá vốn' }, { key: 'pl1', label: 'PL1' }, { key: 'ty_le_co', label: '% CO/RE', kind: 'line' }] },
      { id: 'c_nhom', title: 'GMV theo nhóm hàng', type: 'bar', table: 'cpv_nhom', x: 'thang', series: [{ key: 'gmv_giftcard', label: 'Giftcard', stack: true }, { key: 'gmv_tien_ich', label: 'Tiện ích', stack: true }, { key: 'gmv_steam', label: 'Steam', stack: true }] },
      { id: 'c_ngay', title: 'GMV theo ngày — tháng đang chạy', type: 'bar', table: 'cpv_ngay', x: 'ngay', series: [{ key: 'co', label: 'Giá vốn', stack: true }, { key: 'pl1', label: 'PL1', stack: true }, { key: 'ty_le_co', label: '% CO/GMV', kind: 'line' }] },
      { id: 'c_don', title: 'Số đơn theo tháng', type: 'bar', table: 'cpv_thang', x: 'thang', series: [{ key: 'so_don', label: 'Số đơn' }] },
    ],
  },
  ...CPV_TEAM_REPORTS,
  ...CPV_SAN_REPORTS,
