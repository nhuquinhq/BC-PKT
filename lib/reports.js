...
export const REPORTS = [
  ...
  /* ---------------------------------------------------------- PKT3 */
  {
    ...
    defaultPeriod: 'month',
    source: 'HubPay · MISA · Sao kê ví sàn · Balance',
    purpose: [
  ...
  /* ---------------------------------------------------------- PKT6 */
  {
    ...
    periods: ['week', 'month', 'quarter'],
    defaultPeriod: 'month',
    source: 'Lịch sử ví sàn (Balance · HubPay) · GoX / ODP',
    purpose: [
      'SPDV nào / Team nào đang tạo ra lợi nhuận thật, cái nào càng bán càng lỗ?',
      'Mỗi đơn hàng trung bình mang về bao nhiêu doanh thu (ARPO) và bao nhiêu lợi nhuận?',
      'Tỉ lệ đơn thất bại & hoàn hủy của từng Team/SPDV là bao nhiêu — đang mất tiền ở đâu?',
    ],
    kpis: [
      { key: 'tong_don', code: 'ORD', label: 'Tổng số đơn thành công', type: 'num' },
      { key: 'arpo', code: 'ARPO', label: 'Doanh thu bình quân / đơn', type: 'money' },
      { key: 'ln_don', code: 'PPO', label: 'Lợi nhuận bình quân / đơn', type: 'money', tone: 'auto' },
      { key: 'ty_le_fail', code: '%FAIL', label: 'Tỉ lệ đơn thất bại', type: 'pct', tone: 'loss' },
      { key: 'ty_le_hoan', code: '%RR', label: 'Tỉ lệ hoàn hủy', type: 'pct', tone: 'loss' },
      { key: 'bien_pl2', code: '%PL2', label: 'Biên PL2 toàn hệ', type: 'pct', tone: 'auto' },
      { key: 'so_spdv_lo', code: 'LOSS', label: 'Số SPDV đang lỗ', type: 'num', tone: 'loss' },
      { key: 'gia_tri_mat', code: 'LEAK', label: 'Giá trị mất do fail & hoàn hủy', type: 'money', tone: 'loss' },
    ],
    tables: [
      {
        id: 'lich_su_vi_san',
        title: 'Lịch sử ví sàn — dữ liệu nguồn',
        hint: 'Xuất từ Balance / HubPay: mọi giao dịch nạp, rút, phí, hold theo từng ví/sàn. CPV bóc từ bảng này.',
        columns: [
          { key: 'ngay', label: 'Ngày', type: 'date' },
          { key: 'vi_san', label: 'Ví / Sàn', type: 'text' },
          { key: 'loai_tien', label: 'Loại tiền', type: 'text' },
          { key: 'loai_gd', label: 'Loại giao dịch', type: 'text' },
          { key: 'so_tien', label: 'Số tiền nguyên tệ', type: 'num' },
          { key: 'phi', label: 'Phí', type: 'num' },
          { key: 'ty_gia', label: 'Tỉ giá quy đổi', type: 'rate' },
          { key: 'quy_doi_vnd', label: 'Quy đổi VND', type: 'money' },
          { key: 'so_du_sau', label: 'Số dư sau GD', type: 'num' },
          { key: 'ghi_chu', label: 'Ghi chú', type: 'text' },
        ],
      },
      {
        id: 'cpv_matrix',
        title: 'Ma trận Chi phí – Sản lượng – Lợi nhuận theo Team × SPDV',
        hint: 'Xếp loại: Sinh lời tốt / Hoà vốn / Càng bán càng lỗ.',
        columns: [
          { key: 'team', label: 'Team / BU', type: 'text' },
          { key: 'spdv', label: 'SPDV', type: 'text' },
          { key: 'san', label: 'Sàn', type: 'text' },
          { key: 'so_don', label: 'Số đơn', type: 'num' },
          { key: 'don_fail', label: 'Đơn fail', type: 'num' },
          { key: 'don_hoan', label: 'Đơn hoàn hủy', type: 'num' },
          { key: 'gmv', label: 'GMV', type: 'money' },
          { key: 're', label: 'RE', type: 'money' },
          { key: 'cogs', label: 'COGS', type: 'money' },
          { key: 'pl1', label: 'PL1', type: 'money' },
          { key: 'pl2', label: 'PL2', type: 'money' },
          { key: 'bien_pl2', label: '% PL2/RE', type: 'pct' },
          { key: 'arpo', label: 'ARPO', type: 'money' },
          { key: 'ln_don', label: 'LN / đơn', type: 'money' },
          { key: 'xep_loai', label: 'Xếp loại', type: 'text' },
        ],
      },
      {
        id: 'cpv_leak',
        title: 'Bảng thất thoát: đơn fail & hoàn hủy',
        columns: [
          { key: 'team', label: 'Team', type: 'text' },
          { key: 'spdv', label: 'SPDV', type: 'text' },
          { key: 'nguyen_nhan', label: 'Nguyên nhân', type: 'text' },
          { key: 'so_don', label: 'Số đơn', type: 'num' },
          { key: 'gia_tri_mat', label: 'Giá trị mất', type: 'money' },
          { key: 'tren_gmv', label: '% / GMV', type: 'pct' },
          { key: 'trach_nhiem', label: 'Bộ phận chịu trách nhiệm', type: 'text' },
          { key: 'hanh_dong', label: 'Hành động khắc phục', type: 'text' },
        ],
      },
    ],
    charts: [
      { id: 'c_scatter', title: 'Doanh thu và biên lợi nhuận theo SPDV', type: 'scatter', table: 'cpv_matrix', x: 're', y: 'bien_pl2', label: 'spdv' },
      { id: 'c_pl2_team', title: 'PL2 theo Team', type: 'bar', table: 'cpv_matrix', x: 'team', series: [{ key: 'pl2', label: 'PL2' }] },
    ],
  },
  ...
  /* ---------------------------------------------------------- PKT8 */
  {
    ...
    short: 'CPV BE HQS',
    nav: 'CPV BE HQS (backend)',
    tier: 2,
    sla: 'Ngày 5',
    periods: ['week', 'month', 'quarter'],
    defaultPeriod: 'month',
    source: 'Lịch sử ví sàn (Balance · HubPay) · hệ thống BE HQS',
    purpose: [
      'Khối backend HQS (GoX, Hub, HubPay, HubATM, Balance…) tiêu bao nhiêu chi phí vận hành mỗi kỳ?',
      'CPV BE phân bổ cho BU / SPDV nào, chiếm bao nhiêu % trên GMV và PL2?',
      'Dòng tiền qua ví sàn (nạp – rút – phí – hold) có khớp sổ sách không, lệch ở đâu?',
    ],
    kpis: [
      { key: 'tong_cpv_be', code: 'CPV', label: 'Tổng CPV BE kỳ này', type: 'money', tone: 'loss' },
      { key: 'cpv_tren_gmv', code: '%GMV', label: '% CPV BE / GMV', type: 'pct', tone: 'warn' },
      { key: 'cpv_tren_pl2', code: '%PL2', label: '% CPV BE / PL2', type: 'pct', tone: 'warn' },
      { key: 'phi_vi_san', code: 'FEE', label: 'Phí ví sàn (rút · chuyển)', type: 'money', tone: 'loss' },
      { key: 'so_du_vi', code: 'BAL', label: 'Số dư ví sàn cuối kỳ (VND)', type: 'money' },
      { key: 'chenh_doi_soat', code: 'GAP', label: 'Chênh lệch đối soát ví', type: 'money', tone: 'flip' },
      { key: 'bien_dong_cpv', code: 'Δ', label: 'Biến động CPV so kỳ trước', type: 'pct', tone: 'auto' },
    ],
    tables: [
      {
        id: 'lich_su_vi_san',
        title: 'Lịch sử ví sàn — dữ liệu nguồn',
        hint: 'Xuất từ Balance / HubPay: mọi giao dịch nạp, rút, phí, hold theo từng ví/sàn.',
        columns: [
          { key: 'ngay', label: 'Ngày', type: 'date' },
          { key: 'vi_san', label: 'Ví / Sàn', type: 'text' },
          { key: 'loai_tien', label: 'Loại tiền', type: 'text' },
      ...
      {
        id: 'cpv_be_he_thong',
        title: 'CPV theo hệ thống BE HQS',
        hint: 'Chi phí bóc theo từng hệ thống backend, phân bổ về BU/SPDV sử dụng.',
        columns: [
          { key: 'he_thong', label: 'Hệ thống BE', type: 'text' },
          { key: 'khoan_muc', label: 'Khoản mục chi phí', type: 'text' },
          { key: 'ky', label: 'Kỳ', type: 'text' },
          { key: 'chi_phi', label: 'Chi phí', type: 'money' },
          { key: 'phan_bo_bu', label: 'Phân bổ cho BU / SPDV', type: 'text' },
          { key: 'tren_gmv', label: '% / GMV', type: 'pct' },
          { key: 'tren_pl2', label: '% / PL2', type: 'pct' },
          { key: 'ghi_chu', label: 'Ghi chú', type: 'text' },
        ],
      },
      {
        id: 'cpv_be_thang',
        title: 'Tổng hợp CPV BE theo tháng',
        columns: [
          { key: 'thang', label: 'Tháng', type: 'text' },
          { key: 'phi_vi_san', label: 'Phí ví sàn', type: 'money' },
          { key: 'phi_ha_tang', label: 'Hạ tầng · server · tool', type: 'money' },
          { key: 'nhan_su_be', label: 'Nhân sự BE', type: 'money' },
          { key: 'khac', label: 'Chi phí khác', type: 'money' },
          { key: 'tong_cpv', label: 'Tổng CPV BE', type: 'money' },
          { key: 'gmv', label: 'GMV kỳ', type: 'money' },
          { key: 'ty_le', label: '% CPV / GMV', type: 'pct' },
        ],
      },
    ],
    charts: [
      { id: 'c_cpv_thang', title: 'CPV BE theo tháng', type: 'bar', table: 'cpv_be_thang', x: 'thang', series: [{ key: 'tong_cpv', label: 'Tổng CPV BE' }] },
      { id: 'c_cpv_ht', title: 'Chi phí theo hệ thống BE', type: 'bar', table: 'cpv_be_he_thong', x: 'he_thong', series: [{ key: 'chi_phi', label: 'Chi phí' }] },
    ],
  },
