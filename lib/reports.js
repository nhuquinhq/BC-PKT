...
import { SAN_DANG_BAN } from '@/lib/cpvDims';
...
/* Tháng ĐANG CHẠY (09/2026) — file chủ đạo đọc live.
   Đo ngày 07/09 trước khi đổi: xuất 3.775.864 byte trong 1,54s và 19,9s,
   37.425 dòng · tiêu đề dòng 4 · 48 cột — đúng khuôn file BE cũ. Đối chiếu
   thêm cho chắc vì cột trùng tên: hai cột "Tỷ giá tuần" thì cột SAU (25.286)
   là REV, cột trước (25.803) là CO; ba cột "Giá vốn" thì cột đứng ngay trước
   "Lợi Nhuận" mới là VND (1,133 tỷ), hai cột kia là số phụ. Cả hai đều khớp
   quy tắc /api/cpv đang dùng nên không phải sửa route.

   ĐỌC THEO HAI ĐƯỜNG. Đường CHÍNH là BẢN CÔNG BỐ, đường dự phòng là /export
   trên file gốc — và thứ tự này quan trọng, đừng đảo lại.

   Đo ngày 15/09. Bài học lớn nhất: ĐO TỪ GITHUB KHÔNG DỰ ĐOÁN ĐƯỢC VERCEL.
     10:14, từ GitHub:  export 5/5 lượt · 1,3–1,6s · công bố 3/5 lượt
     10:47, từ GitHub:  export 1/3 lượt · lượt được mất 163s
     10:53, QUA VERCEL: export 0/3 lượt · HTTP 400 sau 240s mỗi lượt
     11:07, QUA VERCEL: công bố 1/2 lượt · lượt được mất 38,7s
   Tôi từng đặt export làm đường chính theo số đo GitHub, kết quả là trang hỏng
   sạch vì qua Vercel đường đó trả 400. Nguồn nào cũng phải đo QUA VERCEL rồi
   mới chốt.

   Đường gviz thì KHÔNG dùng được cho file này: tải nhanh nhưng bỏ trống tên
   cột "Thành tiền GMV" nên đọc ra tiền bằng 0.

   Cả ba đường đều đang xuống sức đúng như T7, T8 hồi trước. Sau khi nới hạn
   chờ thì 6/6 lượt gọi thử đọc được, nhưng lượt đọc NGUỘI mất tới 158s và 65s
   — tức vẫn còn chậm, chỉ là không còn báo lỗi. Cách dứt điểm là chụp định kỳ
   vào datalake như đã làm với ví (.github/workflows/chup-vi.yml); chưa làm. */
const CPV_SHEET_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vS0Tldb2hmR8TxnYC_un9q0tpgpm3RCyN3O7oJimo17gP4EpagCRwEPwpqMr2BjdZtXeRVOcthy23PX/pubhtml?gid=0&single=true';
const CPV_SHEET_URL_B = 'https://docs.google.com/spreadsheets/d/1WQdk90dz0Ddrh4C7exhuqHfNnD-DH616Ms0vUeDSNZw/edit?gid=0';
/* T7/2026 ĐÃ CHỐT SỔ ngày 17/08 — số nằm trong lib/data/cpv-2026-07.json,
   không đọc live nữa. Bỏ được một file 25.124 dòng khỏi mỗi lượt mở trang. */
/* Các tháng ĐANG chạy đọc live tại đây; tháng đã chốt sổ được đóng gói
   ...
   T5 · T6 · T7/2026 đã chuyển vào datalake. Sang tháng mới: đổi CPV_SHEET_URL
   sang file tháng mới rồi đẩy file tháng vừa qua xuống đây; khi chốt sổ thì
   snapshot vào datalake và bỏ hẳn khỏi danh sách.

   T8/2026 KHÔNG có ở đây dù chưa chốt sổ: đo ngày 07/09 thì bản công bố của
   file BE T8 hỏng 0/5 lượt, lượt nào cũng trả HTTP 307 rỗng sau 110s. Ngày
   26/08 file này còn ra 3/5 lượt trong 0,70s, tức là nó vừa hỏng hẳn chứ
   không phải chậm. Để nó trong danh sách thì mỗi lượt mở trang đốt 150s thử
   lại một file chắc chắn không ra, mà cuối cùng vẫn thiếu T8 — thà thiếu
   nhanh. Muốn có lại số T8 thì cần xuất bản lại file, hoặc tải file về rồi
   đẩy vào datalake như đã làm với T7. */
const CPV_MAINS = [];
...
/* File FINANCIAL REPORT_A10GG (tab Phân bổ) — nguồn cho PKT13 và PKT20.
   Là bảng kết quả kinh doanh THEO THÁNG, không có số theo ngày; tiền về
   chậm ~45 ngày nên tháng vừa qua thường còn RE = 0. Tháng chưa tới để
   số dự trù và bị /api/a10gg loại khỏi phần số thật. */
const A10GG_SHEET_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQvtnyjWuhaqHvTs9rLjzGSDlZKWYww6GROPe3kDW1FBRCFrdLnuYJKXVJslEltOMWy7HpYyOqWRjJG/pubhtml';
/* Khai gid ở CẢ hai chỗ: sheetQuery append 'gid' theo từng file (mặc định
   '0' nếu bỏ trống) và set 'gids' từ qs — để trống gid thì app đi đòi nhầm
   tab 0 và Google trả 401. */
const A10GG_SHEET = { url: A10GG_SHEET_URL, gid: '1620350371', qs: { gids: '1620350371' } };

/* File FINANCIAL REPORT_CHARGING (tab TOA) — nguồn cho PKT14 và PKT20.
   Cũng là bảng kết quả kinh doanh THEO THÁNG. Giá vốn ăn ~92% doanh thu
   nên biên rất mỏng. Tháng chưa tới đã ghi sẵn khoản phân bổ OP nên PL7
   âm — /api/charging loại các tháng đó khỏi số thật. */
const CHARGING_SHEET_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vRoJlJPjNblPMAmO8vrfmC-JWe_dCY7ub4Joqd0exc6oAMnEpDYrUle8QLuBrenFOFGdFtXfTE7TtAR/pubhtml';
const CHARGING_SHEET = { url: CHARGING_SHEET_URL, gid: '894789387', qs: { gids: '894789387' } };
...
/* Ví T7/2026 ĐÃ CHỐT SỔ ngày 17/08 — nằm trong lib/data/vi-2026-07.json.
   Bỏ đọc live cũng vá luôn một lỗi: bản publish của file ví T7 đã đổi cấu
   trúc nên /api/vi trả 502 "Không tìm thấy dòng tiêu đề tab THVí Tiền". */
/* Mã bản công bố của file ví T8 — giữ lại để chạy lại ảnh chụp khi cần:
   chup-vi.yml với kiểu 'pub', mã 2PACX-1vTmigt_mS16GnHQ5qpx2jxv_0F9irT-
   jJt6c3KyfUrTn2AgbZVZ4d5Q0fBvoINSmnMJHm5GcDU58ai2, gid 1425184209. */
/* Các tháng ví ĐANG CHẠY — mỗi tháng một file riêng. Tab ví chỉ ghi SỐ NGÀY
   ...
   vào datalake lib/data/vi-*.json (T4–T7 đã chốt) và bỏ khỏi danh sách này.

   T9 KHÔNG đọc live: đo ngày 07/09 thì Google xuất file ví T9 hụt hẳn — bản
   công bố 0/2 lượt, export 0/3 lượt (lượt nào cũng HTTP 307 rỗng sau 110s),
   chỉ còn đường gviz mà cũng chỉ 1/5 lượt và mất 38–44s. Bốn lần bấm hỏng ba
   thì không dùng được. Thay bằng ảnh chụp mỗi 2 tiếng vào datalake
   lib/data/vi-2026-09.json — xem .github/workflows/chup-vi.yml — rồi để cờ
   hist kéo lên. Ngày nào Google xuất lại được thì thêm một mục vào mains là
   quay về đọc live, /api/vi tự bỏ bản chụp của tháng đã có số live.

   T8 cũng đã vào datalake (lib/data/vi-2026-08.json) ngày 07/09, chụp thẳng từ
   bản công bố vì file ví T8 vẫn xuất bản được. Nên PKT6 giờ KHÔNG đọc live
   file ví nào nữa — mở trang là ra số ngay, không phụ thuộc Google có xuất
   nổi hay không. Đó cũng là câu trả lời cho "dữ liệu nặng quá". */
const VI_LIVE = {
  url: '',
  mains: [],
  hist: true,
...
/* File API trực tiếp từ sàn — hiện chỉ có G1, G2; được ưu tiên khi trùng Order ID.
   Mỗi tháng một file, KHÔNG cuốn chiếu: đo ngày 07/09 thì file T8 có 352 dòng
   và toàn bộ là 08/2026, không có dòng T9 nào. Thiếu file tháng mới là báo cáo
   hụt hẳn phần đơn tự động — tháng 7 phần này là 781.906.237 đ trên tổng
   8.524.646.772 đ, khoảng 9%. */
/* File đơn tự động T9 — cùng file bot đang dùng. Soi ngày 07/09: đúng 31 cột y
   hệt file T8, có cột Giá vốn [23]. Là bản THÔ (không có Thành tiền lẫn cột tỷ
   giá) nên /api/cpv tự quy đổi bằng REV/CO — giống hệt đường đi của file T8. */
const CPV_API_T9_URL = 'https://docs.google.com/spreadsheets/d/1GpOFicbNTmkxCCe9488516LPx31ryZTAVwS9hSxEL7U/edit?gid=0';
/* File API sàn T7 và T8 đã gói vào datalake cùng lượt chốt sổ tháng đó — đọc
   thêm ở đây nữa là cộng dồn hai lần. Chỉ để lại tháng đang chạy. */
const CPV_API = [{ url: CPV_API_T9_URL, gid: '0' }];

/* ---------------------------------------------------------------------
   NGUỒN RIÊNG CHO BOT CPV — "Báo cáo đơn hàng V3"

   Vì sao tách khỏi PKT8: file CPV BE cũ Google xuất bản không nổi nữa. Đo
   ngày 17/08: bản publish của file BE hỏng 5/5 lượt, mỗi lượt treo hết 120s
   rồi trả HTTP 307 rỗng. Hai file V3 do service account đẩy ra thì 5/5 lượt
   thành công, tab đơn hàng 14.600 dòng tải trong 1,05s — nhanh hơn ~300 lần.

   Đây là bản xuất THÔ: có Doanh thu USD, KHÔNG có Thành tiền VND, tỷ giá,
   BU. /api/cpv tự quy đổi VND bằng REV Rate ở lib/data/ty-gia-tuan.json và
   gán BU bằng map sàn→BU ở lib/data/san-bu.json.

   Tab đơn hàng là gid 0, KHÔNG phải gid 1245992231 — gid đó là bảng tổng
   hợp theo ngày, không có Order ID nên /api/cpv không đọc được.

   Web (PKT8, PKT20, trang team và sàn) tạm thời vẫn giữ nguồn cũ; chạy bot
   ổn vài hôm rồi mới chuyển. */
/* Tháng 8 — giữ lại để bot còn số SO CÙNG KỲ cho tới khi T8 chốt sổ vào
   datalake; chốt xong thì bỏ khỏi mains cho nhẹ. */
const CPV_V3_T8_URL = 'https://docs.google.com/spreadsheets/d/1AIJYxsS-SKcE-DVFWom0DqQsGJmvIo03JzBViCLJe3M/edit?gid=0';
const CPV_API_V3_T8_URL = 'https://docs.google.com/spreadsheets/d/1MgKdAkn_shglL0xoduuUnqhKTmJDwpCilUnJgwpmAEQ/edit?gid=0';
/* Tháng 9 — file tháng ĐANG CHẠY. Sang tháng mới: đổi hai dòng này sang file
   tháng mới, đẩy file tháng vừa qua xuống mains. */
const CPV_V3_URL = 'https://docs.google.com/spreadsheets/d/10EBQ6uFLK_wlfzbxw1KQIxu3QZYpWGrO4u35bFcL2yA/edit?gid=0';
const CPV_API_V3_URL = 'https://docs.google.com/spreadsheets/d/1GpOFicbNTmkxCCe9488516LPx31ryZTAVwS9hSxEL7U/edit?gid=0';

export const BOT_CPV_SHEET = {
  url: CPV_V3_URL,
  gid: '0',
  mains: [{ url: CPV_V3_T8_URL, gid: '0' }],
  api: [
    { url: CPV_API_V3_URL, gid: '0' },
    { url: CPV_API_V3_T8_URL, gid: '0' },
  ],
  label: 'Báo cáo đơn hàng V3 09/2026 · tab đơn hàng (gid 0)',
};
...
const cpvTeamReport = (code, bu) => ({
  ...
  sheet: { url: CPV_SHEET_URL, urlb: CPV_SHEET_URL_B, gid: '0', mains: CPV_MAINS, hist: true, mode: 'order_cpv', teamFilter: code, api: CPV_API, label: `Đơn hàng BE · Team ${code}` },
...
/* ------------------------------------------------------------------
   ...
   (Cấu hình → phân quyền trang). Sàn mới thì thêm vào datalake
   lib/data/san-bu.json — danh sách dưới đây tự suy ra từ đó, chỉ lấy
   sàn đang bán hàng. */
const SAN_LIST = SAN_DANG_BAN.map((r) => r.ma).sort();
...
const cpvSanReport = (san) => ({
  ...
  defaultPeriod: 'month',
  source: 'Đơn hàng BE HQS',
  sheet: { url: CPV_SHEET_URL, urlb: CPV_SHEET_URL_B, gid: '0', mains: CPV_MAINS, hist: true, mode: 'order_cpv', sanFilter: san, api: CPV_API, label: `Đơn hàng BE · Sàn ${san}` },
...
export const REPORTS = [
  /* ---------------------------------------------------------- PKT1 */
  {
    ...
    sheet: {
      url: 'https://docs.google.com/spreadsheets/d/e/2PACX-1vRBzYH7dMHHBU1PhVf368oCNlLhKhGFclc4VuH9nucqShlrk5fxbYtUUBUUAbYXzm7c3nXO6P7Yb9vQ/pubhtml?gid=1739295342&single=true',
      /* Tab WEEKLY RATE Ver2 — bảng trải ngang, mỗi tuần một khối cột, có đủ
         REV Rate và CO Rate. Trước đọc tab gid 0 chỉ có bảng tổng quan; tab
         Ver1 đã bỏ, không dùng nữa.
         Đường thứ hai là gviz trên file gốc — đo ngày 07/09 thì cả hai đường
         đều ra và ra cùng 19 tuần với số y hệt, còn /export của file gốc trả
         trang đăng nhập (file chưa mở "ai có link") nên KHÔNG dùng được. */
      gid: '1739295342',
      urls: ['https://docs.google.com/spreadsheets/d/1tMLW_mB8U08CQvE-lPBoLYAbjjs2Cc78t5bLHWTsK48/gviz/tq?tqx=out:csv&gid=1739295342&headers=0'],
      mode: 'weekly_matrix',
      table: 'weekly_rate',
      label: 'HQS - BẢNG TỶ GIÁ HÀNG TUẦN · tab tỷ giá tuần',
    },
    purpose: [
  ...
  /* --------------------------------------------------------- PKT20 */
  {
    ...
    periods: ['day', 'week', 'month'],
    defaultPeriod: 'month',
    source: 'Gộp: đơn hàng BE HQS · lịch sử ví HQS · Daily.Report Ritokey · Báo cáo QLTT · FINANCIAL REPORT A10GG · FINANCIAL REPORT CHARGING',
    /* Bản BE (mặc định) và bản theo VÍ — nút chuyển nằm ở đầu trang.
       Ritokey chỉ có bản BE nên luôn đọc chung một nguồn. */
    sheet: {
      url: CPV_SHEET_URL,
      urlb: CPV_SHEET_URL_B,
      gid: '0',
      mains: CPV_MAINS,
      hist: true,
      mode: 'holdings',
    ...
    sheetA10gg: A10GG_SHEET,
    sheetCharging: CHARGING_SHEET,
    ...
    tables: [
      {
        ...
        totals: true,
        hint: 'GMV = doanh số trên đơn · RE = doanh thu ghi nhận (HQS BE: GMV − phí sàn · bản ví: tiền thực nhận · Ritokey: dòng Doanh thu của file · A10GG: dòng RE của báo cáo kết quả kinh doanh) · CO = giá vốn · PL1 = RE − CO. A10GG chỉ có số theo THÁNG và tiền về chậm ~45 ngày nên tháng vừa qua thường còn RE = 0 dù chi phí đã ghi; tháng dự trù không được cộng vào. File A10GG không tách GMV riêng nên lấy GMV = RE cho đồng nhất, và không ghi số đơn nên cột Số đơn của A10GG bằng 0. Charging cũng chỉ có số theo THÁNG, GMV = RE, và các tháng chưa tới (mới chỉ ghi trước chi phí phân bổ) đã bị loại. HQ Thailand chưa nối nguồn nên đang bằng 0.',
        columns: [
...

  {
    ...
    defaultPeriod: 'month',
    source: '2026 PnL_HQG FINANCIAL REPORT · TOA A10GG · TOA Charging',
    /* Hai dự án đọc trực tiếp, đổ vào bảng pnl_don_vi — xem PnlDonViBoard */
    sheetDonVi: { a10gg: A10GG_SHEET, charging: CHARGING_SHEET },
    purpose: [
    ...
    tables: [
      {
        ...
        hint: 'Giữ nguyên mã chỉ số để đối chiếu với từ điển khái niệm HQS. Bấm dải nút phía trên để đổi đơn vị — Tổng 2 dự án · A10GG · Charging. Các tầng TỰ TÍNH: PL1 = RE − COGS · PL2 = PL1 − SE − ME − OP − FI · PL7 = PL2 − OV − CA − OT, và PL7 tự tính được đối chiếu lại với dòng PL7 ghi trên file (hiện khớp 0 đồng). Dòng để TRỐNG là file không tách được (GMV theo mô hình, CO2/CO3, AR); dòng bằng 0 là hai đơn vị này thật sự không có khoản đó (RR, SF, CF, FG) nên PL2A = PL1. Kỳ trước và %MoM để trống vì dải lọc là tuỳ ý, không có kỳ trước xác định để so.',
        columns: [
  ...
  /* --------------------------------------------------------- PKT30 */
  {
    code: 'PKT30',
    slug: 'sop-ke-toan',
    name: 'SOP Phòng Kế toán',
    short: 'SOP PKT',
    nav: 'SOP Phòng Kế toán',
    tier: 1,
    sla: 'Ver 1.0',
    periods: ['month'],
    defaultPeriod: 'month',
    source: 'Repo sop-pkt · data-noibo.js (bản chép trong lib/sop)',
    /* Trang tài liệu — không đọc Google Sheet, không lọc thời gian */
    sheet: { mode: 'sop', kind: 'nb' },
    purpose: [
      'Từng vị trí trong Phòng Kế toán làm việc theo quy trình nào, ai chịu trách nhiệm khâu nào?',
      'Các vị trí bàn giao cho nhau cái gì, vào lúc nào?',
      'Mỗi quy trình có những chốt kiểm soát nào bắt buộc phải giữ?',
    ],
    kpis: [],
    tables: [],
    charts: [],
  ...
  {
    code: 'PKT31',
    slug: 'sop-lien-phong-ban',
    name: 'SOP Liên phòng ban',
    short: 'SOP LPB',
    nav: 'SOP Liên phòng ban',
    tier: 3,
    sla: 'Ver 1.0',
    periods: ['month'],
    defaultPeriod: 'month',
    source: 'Repo sop-pkt · data-lienphong.js (bản chép trong lib/sop)',
    /* Trang tài liệu — không đọc Google Sheet, không lọc thời gian */
    sheet: { mode: 'sop', kind: 'lp' },
    purpose: [
      'PKT phối hợp với PKD, PCU, các BU và NCC theo những quy trình nào?',
      'Mỗi bên phải giao gì cho PKT, hạn chót khi nào?',
      'Khoản chi nào thuộc nhóm nào, đi qua luồng duyệt nào?',
    ],
    kpis: [],
    tables: [],
    charts: [],
  ...
  /* --------------------------------------------------------- PKT14 */
  {
    code: 'PKT14',
    slug: 'pkt14',
    name: 'Báo cáo CPV Charging',
    short: 'CPV CHARGING',
    nav: 'CPV Charging',
    tier: 3,
    sla: 'Ngày 15',
    /* Chỉ có số theo tháng nên không cho chọn ngày / tuần */
    periods: ['month'],
    defaultPeriod: 'month',
    source: 'FINANCIAL REPORT_CHARGING — tab TOA',
    sheet: {
      ...CHARGING_SHEET,
      mode: 'charging',
      label: 'Charging · Kết quả kinh doanh theo tháng',
    },
    purpose: [
      'Doanh thu, giá vốn và lãi lỗ của dự án Charging từng tháng đang thế nào?',
      'Biên lãi mỏng tới mức nào, và tháng nào giá vốn ăn sâu nhất vào doanh thu?',
      'Chi phí ngoài giá vốn (OP, OV) đang chiếm bao nhiêu phần doanh thu?',
    ],
    kpis: [
      { key: 're', code: 'RE', label: 'Doanh thu', type: 'money' },
      { key: 'co', code: 'CO', label: 'Giá vốn (COGS)', type: 'money', tone: 'loss' },
      { key: 'pl1', code: 'PL1', label: 'Lãi gộp (RE − CO)', type: 'money', tone: 'auto' },
      { key: 'tong_chi', code: 'CHI', label: 'Tổng chi', type: 'money', tone: 'loss' },
      { key: 'pl2', code: 'PL2', label: 'PL2', type: 'money', tone: 'auto' },
      { key: 'pl7', code: 'PL7', label: 'Lãi/Lỗ (PL7)', type: 'money', tone: 'auto' },
      { key: 'ty_le_co', code: '%CO', label: '% Giá vốn / Doanh thu', type: 'pct', tone: 'warn' },
      { key: 'bien_pl7', code: '%PL7', label: 'Biên lãi/lỗ (PL7/RE)', type: 'pct', tone: 'auto' },
    ],
    tables: [
      {
        id: 'ch_thang',
        title: 'Kết quả kinh doanh theo THÁNG — số thật',
        totals: true,
        hint: 'Đọc thẳng từ các dòng RE · COGS · PL2 · PL7 của tab TOA. Giá vốn chiếm khoảng 92% doanh thu nên biên rất mỏng — một điểm phần trăm giá vốn đổi chiều là lãi đổi theo. Các tháng chưa tới đã bị loại khỏi bảng này.',
        columns: [
          { key: 'thang', label: 'Tháng', type: 'text' },
          { key: 're', label: 'RE — Doanh thu', type: 'money' },
          { key: 'co', label: 'CO — Giá vốn', type: 'money' },
          { key: 'pl1', label: 'PL1 = RE − CO', type: 'money' },
          { key: 'pl2', label: 'PL2', type: 'money' },
          { key: 'pl7', label: 'PL7 — Lãi/Lỗ', type: 'money' },
          { key: 'ty_le_co', label: '% CO/RE', type: 'pct', totalOf: ['co', 're'] },
          { key: 'bien_pl7', label: 'Biên PL7/RE', type: 'pct', totalOf: ['pl7', 're'] },
        ],
      },
      {
        id: 'ch_chi_phi',
        title: 'Chi phí theo KHỐI',
        totals: true,
        hint: 'COGS = giá vốn · SE = bán hàng · ME = marketing · OP = vận hành · OV = chi phí chung phân bổ · FI = tài chính · CA = vốn · OT = khác.',
        columns: [
          { key: 'thang', label: 'Tháng', type: 'text' },
          { key: 'co', label: 'COGS', type: 'money' },
          { key: 'se', label: 'SE', type: 'money' },
          { key: 'me', label: 'ME', type: 'money' },
          { key: 'op', label: 'OP', type: 'money' },
          { key: 'ov', label: 'OV', type: 'money' },
          { key: 'tong_chi', label: 'Tổng chi', type: 'money' },
        ],
      },
      {
        id: 'ch_chua_toi',
        title: 'Tháng CHƯA TỚI — chi phí ghi trước, không tính vào tổng',
        hint: 'Các tháng chưa phát sinh doanh thu nhưng file đã ghi sẵn khoản phân bổ OP nên lãi/lỗ âm. Đây là chi phí ghi trước theo kế hoạch, không phải lỗ đã xảy ra — tuyệt đối không cộng vào số thực hiện của dự án hay của tập đoàn.',
        columns: [
          { key: 'thang', label: 'Tháng', type: 'text' },
          { key: 're', label: 'RE', type: 'money' },
          { key: 'co', label: 'CO', type: 'money' },
          { key: 'op', label: 'OP ghi trước', type: 'money' },
          { key: 'pl7', label: 'PL7', type: 'money' },
        ],
      },
    ],
    charts: [
      {
        /* Cột chồng theo quy ước dự án: Doanh thu = Giá vốn + Lãi gộp,
           nên chiều cao cột chính là doanh thu. Đường là biên PL7. */
        id: 'c_ch_trend',
        title: 'Doanh thu = Giá vốn + Lãi gộp (cột chồng) · Lãi/Lỗ (đường)',
        type: 'bar',
        table: 'ch_thang',
        x: 'thang',
        series: [
          { key: 'co', label: 'Giá vốn (COGS)', stack: true, color: '#D96F00' },
          { key: 'pl1', label: 'Lãi gộp (RE − CO)', stack: true, color: '#189BD8' },
          { key: 'bien_pl7', label: '% Lãi/Lỗ trên doanh thu', kind: 'line', color: '#00A651' },
        ],
        tip: [
          { key: 're', label: 'Doanh thu (RE)' },
          { key: 'pl7', label: 'Lãi/Lỗ (PL7) — số tiền' },
          { key: 'tong_chi', label: 'Tổng chi' },
        ],
      },
      {
        id: 'c_ch_pct',
        title: '% Tỉ trọng chi phí trên doanh thu và biên lãi/lỗ',
        type: 'bar',
        table: 'ch_thang',
        x: 'thang',
        pct: true,
        series: [
          { key: 'ty_le_co', label: '% COGS/RE', color: '#D96F00' },
          { key: 'ty_le_op', label: '% OP/RE', color: '#1B75BB' },
          { key: 'ty_le_ov', label: '% OV/RE', color: '#189BD8' },
          { key: 'bien_pl7', label: '% PL7/RE (Contribution Margin)', kind: 'line', color: '#00A651' },
        ],
        tip: [
          { key: 're', label: 'Doanh thu (RE)' },
          { key: 'pl7', label: 'Lãi/Lỗ (PL7) — số tiền' },
  ...
  /* --------------------------------------------------------- PKT13 */
  {
    ...
    periods: ['month'],
    defaultPeriod: 'month',
    source: 'FINANCIAL REPORT_A10GG — tab Phân bổ',
    sheet: {
      ...A10GG_SHEET,
      mode: 'a10gg',
      label: 'A10GG · Kết quả kinh doanh theo tháng',
    },
    purpose: [
      'Doanh thu, giá vốn và lãi lỗ của A10GG từng tháng đang thế nào?',
      'Chi phí của A10GG đang nằm ở khối nào — giá vốn, bán hàng, marketing hay vận hành?',
      'Tháng nào đã ghi chi phí nhưng tiền bán hàng chưa về?',
    ],
    kpis: [
      { key: 're', code: 'RE', label: 'Doanh thu', type: 'money' },
      { key: 'co', code: 'CO', label: 'Giá vốn (COGS)', type: 'money', tone: 'loss' },
    ...
    tables: [
      {
        ...
        title: 'Kết quả kinh doanh theo THÁNG — số thật',
        totals: true,
        hint: 'Đọc thẳng từ các dòng RE · COGS · TỔNG CHI · LÃI/LỖ (PL7) của tab Phân bổ. A10GG tiền về chậm khoảng 45 ngày nên tháng vừa qua thường đã có chi phí mà doanh thu còn 0 — đó là độ trễ thu tiền, không phải ngừng bán. Các tháng dự trù đã bị loại khỏi bảng này.',
        columns: [
          { key: 'thang', label: 'Tháng', type: 'text' },
          { key: 're', label: 'RE — Doanh thu', type: 'money' },
          { key: 'co', label: 'CO — Giá vốn', type: 'money' },
      ...
      },
      {
        ...
        title: 'Chi phí theo KHỐI',
        totals: true,
        hint: 'SE = chi phí bán hàng · ME = marketing · OP = vận hành · OV = chi phí chung phân bổ. Cộng lại cùng COGS ra Tổng chi.',
        columns: [
          { key: 'thang', label: 'Tháng', type: 'text' },
          { key: 'co', label: 'COGS — Giá vốn', type: 'money' },
          { key: 'se', label: 'SE — Bán hàng', type: 'money' },
          { key: 'me', label: 'ME — Marketing', type: 'money' },
          { key: 'op', label: 'OP — Vận hành', type: 'money' },
          { key: 'ov', label: 'OV — Chi phí chung', type: 'money' },
          { key: 'tong_chi', label: 'Tổng chi', type: 'money' },
        ],
      },
      {
        id: 'a10_du_tru',
        title: 'Tháng DỰ TRÙ — không tính vào tổng',
        hint: 'Các tháng chưa tới đang để số dự trù trên file. Liệt kê ở đây để tham khảo kế hoạch, tuyệt đối không cộng vào số thực hiện của team hay của tập đoàn.',
        columns: [
          { key: 'thang', label: 'Tháng', type: 'text' },
          { key: 're', label: 'RE dự trù', type: 'money' },
          { key: 'co', label: 'CO dự trù', type: 'money' },
          { key: 'tong_chi', label: 'Tổng chi dự trù', type: 'money' },
          { key: 'pl7', label: 'PL7 dự trù', type: 'money' },
        ],
      },
    ],
    charts: [
      {
        /* Cột chồng theo đúng quy ước sẵn có của dự án (PKT20 · PKT12 ·
           PKT21): Doanh thu = Giá vốn + Lãi gộp, nên CHIỀU CAO CỘT chính
           là doanh thu. Không chồng thẳng RE lên CO vì giá vốn đã nằm
           trong doanh thu — chồng kiểu đó cột sẽ cao thành RE + CO, một
           con số không có nghĩa.
           Đường là % lãi/lỗ trên doanh thu (trục phải, tự co giãn nên
           hiện được cả phần âm); số tiền lãi/lỗ nằm trong hộp chú giải
           khi rê chuột. */
        id: 'c_a10_trend',
        title: 'Doanh thu = Giá vốn + Lãi gộp (cột chồng) · Lãi/Lỗ (đường)',
        type: 'bar',
      ...
      {
        /* Bản TOA: cột chồng các khối chi phí, đường doanh thu đè lên —
           nhìn ra ngay tháng nào chi vượt thu. */
        id: 'c_a10_toa',
        title: 'TOA — Cơ cấu chi phí theo tháng, so với doanh thu',
        type: 'bar',
        table: 'a10_thang',
        x: 'thang',
        /* Ép màu từng khối: để mặc định thì 'co' rơi vào màu dự phòng
           TRÙNG với đường doanh thu, còn ở biểu đồ % thì ty_le_ov trùng
           ty_le_co. Giữ cùng một màu cho cùng một khối ở cả hai biểu đồ
           để đọc đối chiếu được. */
        series: [
          { key: 'co', label: 'COGS — Giá vốn', stack: true, color: '#D96F00' },
          { key: 'se', label: 'SE — Bán hàng', stack: true, color: '#7E9C00' },
          { key: 'me', label: 'ME — Marketing', stack: true, color: '#00A99D' },
          { key: 'op', label: 'OP — Vận hành', stack: true, color: '#1B75BB' },
          { key: 'ov', label: 'OV — Chi phí chung', stack: true, color: '#189BD8' },
          { key: 're', label: 'RE — Doanh thu', kind: 'line', num: true, color: '#00A651' },
        ],
      },
      {
        /* Bản %TOA: mỗi khối chi phí một cột riêng (KHÔNG chồng) vì tổng
           tỉ trọng có tháng vượt 190% — chồng lên sẽ bị cắt ngọn ở 100%
           và làm sai hình. Đường PL7/RE nằm trục phải, tự co giãn nên
           hiện được cả phần âm. */
        id: 'c_a10_pct',
        title: '% TOA — Tỉ trọng chi phí trên doanh thu và biên lãi/lỗ',
        type: 'bar',
        table: 'a10_thang',
        x: 'thang',
        pct: true,
        series: [
          { key: 'ty_le_co', label: '% COGS/RE', color: '#D96F00' },
          { key: 'ty_le_se', label: '% SE/RE', color: '#7E9C00' },
          { key: 'ty_le_me', label: '% ME/RE', color: '#00A99D' },
          { key: 'ty_le_op', label: '% OP/RE', color: '#1B75BB' },
          { key: 'ty_le_ov', label: '% OV/RE', color: '#189BD8' },
          { key: 'bien_pl7', label: '% PL7/RE (Contribution Margin)', kind: 'line', color: '#00A651' },
        ],
